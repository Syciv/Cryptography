...
namespace Cryptography.Algorithms.Hash
{
    public static class Stribog
    {
        // Сумма по модулю 2^512
        public static byte[] SummMod512(byte[] a, byte[] b)
        {
        ...
        // Xor
        public static byte[] Stribog_X(byte[] k, byte[] a)
        {
            byte[] result = new byte[64];
            for(int i = 0; i < 64; i++)
            {
        ...
        // Подстановки
        public static byte[] Stribog_S(byte[] bytes)
        {
            byte[] Pi =
            {
                ...
                136, 217, 231, 137, 225, 27, 131, 73, 76, 63, 248, 254, 141, 83, 170,
                144, 202, 216, 133, 97, 32, 113, 103, 164, 45, 43, 9, 91, 203, 155, 37,
                208, 190, 229, 108, 82, 89, 166, 116, 210, 230, 244, 180, 192, 209, 102,
        ...
        // Перестановки
        public static byte[] Stribog_P(byte[] bytes)
        {
            byte[] t =
                 {0, 8, 16, 24, 32, 40, 48, 56, 1,9, 17, 25, 33, 41, 49,
                57, 2, 10, 18, 26, 34, 42, 50, 58, 3, 11, 19, 27, 35, 43,
                51, 59, 4, 12, 20, 28, 36, 44, 52, 60, 5, 13, 21, 29, 37,
        ...
        // Преобразование L для последовательности из 64 бит
        public static ulong Stribog_L64(ulong bits)
        {
            ulong[] Vec4 = {
            ...
            ulong c = 0;
            ulong ci;
            // Для получения нужного бита
            ulong p = 0x8000000000000000;
            for (int i = 63; i>=0; i--)
            {
               
                int bit = (bits & (ulong)p) == 0? 0: 1; // Поменять?
        ...
        // Преобразование L для 512 бит
        public static byte[] Stribog_L(byte[] bytes)
        {
            ulong bits;
            ulong mult = 1;
            byte[] result = new byte[64];
            ...
            for (int i = 0; i < 8; i++)
            {
                mult = 1;
                bits = 0;
                for (int j = 0; j < 8; j++, mult *= 256)
                {
                    bits += mult * bytes[i*8 + 7 - j];
                }

                bits = Stribog_L64(bits);

                var hihi = BitConverter.GetBytes(bits);
                for (int j = 0; j < 8; j++)
                {
                    result[ri] = hihi[7 - j];
                    ri++;
        ...
        // Раундовый ключ
        public static byte[] IterKey(byte[] K, int i)
        {
            byte[][] C = {
                new byte[64]{
        ...
        // Преобразование g
        public static byte[] Stribog_g(byte[] N, byte[] m, byte[] h)
        {
            var K = Stribog_X(N, h);
            K = Stribog_S(K);
            K = Stribog_P(K);
        ...
        public static byte[] GetHash(byte[] bytes)
        {
            ...
            byte[] byte512 = new byte[64];
            for (int i = 0; i < 64; i++)
            {
                zero[i] = h[i] = N[i] = E[i] = byte512[i] = 0x00;
            }
            ...
            int num = M1.Length;
            byte[] m = new byte[64];
            // Преобразование слишком длинного текста
            while (num * 8 >= 512)
            {
                for (int i = bytes.Length - 64, j = 0; i < bytes.Length; i++, j++)
                {
                    m[j] = M1[i];       
                }
                h = Stribog_g(N, m, h);
            ...
            // Дополнение
            for (int i=0; i < (64 - M1.Length - 1); i++)
            {
                m[i] = 0;
            }
            m[64 - M1.Length - 1] = 0x01;
            for (int i = 64 - M1.Length, j=0; i < m.Length; i++,j++)
            {
                m[i] = M1[j];
            }
            //
            ...
            // Перевод размера M1 в байты для сложения
            byte[] M_length = new byte[64];
            for (int i=0; i < 64; i++)
            {
                M_length[i] = 0x00;
            }
            ...
            M_length[63] = (byte)(len % 256);
            M_length[62] = (byte)(len / 256);
            // 
