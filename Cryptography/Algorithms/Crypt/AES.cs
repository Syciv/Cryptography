...
namespace Cryptography.Algorithms.Crypt
{
    public static class AES
    {
        ...
        static void Output10(byte[,] text)
        {
            ...
            {
                for(int j = 0; j < 4; j++)
                    Console.Write(Convert.ToString(text[j,i], 10) + " ");
            }
            Console.WriteLine();
        }
        ...
        static void Output10Row(byte[] text)
        {
        ...
        public static byte[,] MixColumns(byte[,] state, bool enc)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                if (enc)
                {
                    s0 = (byte)(GF.Mult(state[0, i], 2) ^ GF.Mult(state[1, i], 3) ^ state[2, i] ^ state[3, i]);
                    s1 = (byte)(state[0, i] ^ GF.Mult(state[1, i], 2) ^ GF.Mult(state[2, i], 3) ^ state[3, i]);
                    s2 = (byte)(state[0, i] ^ state[1, i] ^ GF.Mult(state[2, i], 2) ^ GF.Mult(state[3, i], 3));
                    s3 = (byte)(GF.Mult(state[0, i], 3) ^ state[1, i] ^ state[2, i] ^ GF.Mult(state[3, i], 2));
                }
                else
                {
                    s0 = (byte)(GF.Mult(state[0, i], 0x0e) ^ GF.Mult(state[1, i], 0x0b) ^ GF.Mult(state[2, i], 0x0d) ^ GF.Mult(state[3, i], 0x09));
                    s1 = (byte)(GF.Mult(state[0, i], 0x09) ^ GF.Mult(state[1, i], 0x0e) ^ GF.Mult(state[2, i], 0x0b) ^ GF.Mult(state[3, i], 0x0d));
                    s2 = (byte)(GF.Mult(state[0, i], 0x0d) ^ GF.Mult(state[1, i], 0x09) ^ GF.Mult(state[2, i], 0x0e) ^ GF.Mult(state[3, i], 0x0b));
                    s3 = (byte)(GF.Mult(state[0, i], 0x0b) ^ GF.Mult(state[1, i], 0x0d) ^ GF.Mult(state[2, i], 0x09) ^ GF.Mult(state[3, i], 0x0e));
                }
        ...
        public static byte[,] KeyExpansion(byte[,] key)
        {
            ...
            for(int col = 4; col < Nb * (Nr + 1); col++)
            {
                if(col % 4 == 0)
                {
                    ...
                    {   
                        byte s = (byte)(keyShedule[row, col - 4] ^ sPrev[row] ^ rCon[row, Nk - 4]);
        ...
        {
            int last = text.Length % 16 != 0 ? 16 - text.Length % 16 : 0;
            ...
            byte[,] testKey =
           {
            ...
            //key = testKey;
            //Console.WriteLine("Ключ енкрипт:");
            //Output16all(key);
            int i = 0;
        ...
        public static byte[] Decrypt(byte[] text)
        {
            ...
            //key = testKey;
            //Console.WriteLine("Ключ декрипт:");
            //Output16all(key);
            int i = 0;
